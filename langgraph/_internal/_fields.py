...
def _is_optional_type(type_: Any) -> bool:
    ...
    if getattr(type_, "__origin__", None) is not None and getattr(type_, "__args__", ()):
        origin = get_origin(type_)
        if origin is Optional:
            return True
        if origin is Union:
            # Check for NoneType first before recursing
            for arg in type_.__args__:
                if arg is type(None):
                    return True
            return any(_is_optional_type(arg) for arg in type_.__args__)
        if origin is Annotated:
            return _is_optional_type(type_.__args__[0])
        return origin is None
    if getattr(type_, "__bound__", None) is not None:
